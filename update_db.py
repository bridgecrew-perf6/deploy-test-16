...
all = []
for i in range(0, length):
    all.append([id[i], chat_id[i], phone[i], uid[i], car_num[i], car_num_short[i], tsm[i], car_driver[i], car_driver_short[i],
...
base.commit()
base.execute('CREATE TABLE IF NOT EXISTS first_driver(id, chat,phone,uid PRIMARY KEY,car_num,car_num_short,tsm,'
